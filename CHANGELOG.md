...
### Fixed
- **图标引用对齐 Oasisic-Icons 最新结构** — 4 个 config 各更新 8 组（每文件 15 行）：OneDrive → `Microsoft/OneDrive/`、Disney → `Media/DisneyPlus/`（上游 Disney 已移除）、LINETV → `Media/LINETV/`、网易云音乐 → `Music/NetEaseCloudMusic/`、Twitch → `Game/Twitch/`、friDay video → `Media/friDayVideo/`、爱奇艺 → `Media/iQIYI/`；HBO 组补 `Media/HBOMAX/HBOMAX.png` 图标（上游已无 HBO 目录）
- **`match_icons.py` ICON_OVERRIDES 对齐新结构** — `friDay video` / `Disney` / `HBO` / `网易云音乐` 显式指向新路径，`OneDrive` 改指 `Microsoft/OneDrive/`（映射 113/121，emoji 组跳过 8，未匹配 0）
- **`batch_update.py` 通知缩略图死链修复** — `icons/Mihomo/Mihomo.png`（Oasisic-Icons 无此品牌）→ `icons/Tool/GitHub/GitHub.png`

验证：`verify_configs` 4/4 PASS（24 项）· `verify_rulesets` 130 PASS · 单测 116 OK · 生成器幂等 ×4 · 452 条 icon URL 0 个 404
